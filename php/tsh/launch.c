...
//    printf("\ncmd is %s \n",cmd);
    ...
    strcpy(in.cmd,cmd);
       ...
       this_op = 5 + TSH_OP_MIN - 1 ;
       if (this_op >= TSH_OP_MIN && this_op <= TSH_OP_MAX)
	   {
		   this_op = htons(this_op) ;
		   tshsock = connectTsh(atoi(getenv("TSH_PORT"))) ;
		   // Send this_op to TSH
...
void OpShell()
{
	...
	printf("\nEnter command(s)$");
	getchar();
	fgets(in.cmd, sizeof(in.cmd), stdin);
	printf("Read cmd(%s)\n",in.cmd);*/

	in.host = out.host;
	in.port = out.port;
//	printf("Before sending TSH port(%d) host(%lu) cmd(%s)\n", in.port, in.host, in.cmd);
	// send shell_in
	...
	// read shell_out
	if (!readn(tshsock, (char *)&sout, sizeof(sout)))
    {
