...
{ 	struct sigaction sa;
	...
	while (received < N) {
		...
		alarm(N/P + 1); // Wait time = f(N). Only trigger at the very end
		//printf("Tuple Name :: %s", tpname);
	...
	sprintf(query, "insert into ScalabilityLog (Timestamp,Server,ElapsedTime,NumOfHosts,ChunkSize,Size,MFLOPS) values (\"%s\",\"%s\",%f,%d,%d,%d,%f)", timeStamp,host,t1/1000000,P,G,N+1,(float) N*N/t1*N);
